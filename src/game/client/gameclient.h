...
public:
	...
	int m_PrismTriggerTargetId = -1;
	int m_PrismTriggerLastTick = -1000;
	bool m_PrismTriggerHookOwned = false;
	bool m_PrismTriggerMatched = false;
	bool m_PrismTriggerOutputActive = false;
	bool m_PrismTriggerDebugReady = false;
	int m_PrismTriggerCandidateCount = 0;
	vec2 m_PrismTriggerPredictedPos = vec2(0, 0);
	vec2 m_PrismTriggerDebugOrigin = vec2(0, 0);
	vec2 m_PrismTriggerDebugDirection = vec2(1, 0);
	bool PrismComposeAssist(CNetObj_PlayerInput &Input, int ManualDirection, bool ManualJump, bool MacroDirectionOwned, bool MacroFirePulse, bool MacroHookOwned);
