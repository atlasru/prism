...
def main():
    ...
    candidates = list(build.glob('Prism-0.1.0-win64.zip'))
    ...
    filename = f'prism-v0.1.0-windows-x64-{commit[:12]}.zip'
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        package = next(path.parent for path in temp.rglob('Prism.exe'))
    ...
    (output / 'archive-verification.json').write_text(json.dumps(report, indent=2) + '\n')
